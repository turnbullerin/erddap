...
public class TranslateMessages {
  ...
  public static String[] localizedLanguageNames = {
    // arbitrarily selected from most commonly used:
    // https://www.visualcapitalist.com/100-most-spoken-languages/
    // Chinese-CN=Simplified  Chinese-TW=Traditional -- I wanted shorter names
    // I got main Indian languages, but not all.
  ...
  /**
   * Method provided by Google to create a glossary. THIS IS NO LONGER USED BECAUSE THE GLOSSARY IS
   * NO LONGER USED.
   ...
   *
   * <p>[Bob initially had permissions problems with this. See solution in setup comments at top.]
   *
   ...
   * @param languageCodes Language code in the glossary, order from left to right in
   *     erddap-glossary.csv
   * @throws Exception if trouble
   */
  @Deprecated
  private static void createGlossary(
      String projectId, String glossaryId, List<String> languageCodes) throws Exception {
  ...
  /**
   * Delete the target glossary. Method provided by Google. THIS IS NO LONGER USED BECAUSE THE
   * GLOSSARY IS NO LONGER USED.
   *
  ...
  /**
   * This updates the glossary by deleting it and recreating one with the erddap-glossary.csv file
   * in the Google Cloud Bucket. THIS IS NO LONGER USED BECAUSE THE GLOSSARY IS NO LONGER USED.
   *
   * @throws Exception if trouble
   */
  @Deprecated
  ...
  /**
   * This method will translate the input text to the target language and output a String. This
   * method uses a free engine, proven to have poor quality. But it's fast and free.
   *
   ...
   */
  @Deprecated
  private static String translateTextBudgetVer(String langFrom, String langTo, String text)
      throws IOException {
    String urlStr =
        "https://script.google.com/macros/s/AKfycbyoLoNhi3R9QD-yIrdjXMYN_ltP1ctX1vhC_UQioQOMexfXlL3NQr4NUeLIzlPWTyil/exec"
            + "?q="
  ...
  /**
   * This method utilizes Google Cloud Translation-Basic service to translate the input text to the
   * target language. However, this does not support the glossary (keep target words untranslated)
   * feature. Use translateTextV3 instead.
   *
   ...
   */
  @Deprecated
  private static String translateTextV2(
      Translate translator, String langFrom, String langTo, String text, boolean html) {
    translationCounter++;
    return translator
        .translate(
            text,
  ...
  /**
   * This function will detect the tags such that its tag name follows "</EDDGrid.*Example>" or
   * "</EDDTable.*Example>" pattern and add the tags to the doNotTranslateSet. Call this before the
   * translation process CURRENTLY NOT NEEDED because Qi have found all tags following the pattern
   * (as 6/23/21) and hard coded them to the set
   *
   * @throws Exception if trouble
   */
  private static void addDoNotTranslateSet() throws Exception {
    ...
    while (true) {
      ...
      if (xmlReader.topTag().endsWith("Example")
          && (xmlReader.topTag().startsWith("/EDDGrid")
              || xmlReader.topTag().startsWith("/EDDTable"))) {
